...
const logic = (() => {
const DEFAULTS = Object.freeze({ maxTemp: 30, minTemp: 18, minHum: 30, maxHum: 70, heat: true, cold: true, humidity: true, recovery: false, offline: true });
...
async function deliver(ref, value, offline) {
  ...
  const job = await db.runTransaction(async tx => {
    ...
    if(!offline && state.lastSentSignature==='offline' && settings.offline){alert.enabled=true;alert.title='IMNVLab · Estação online';alert.body='A estação voltou a enviar medições. '+alert.body;}
...
exports.weatherPush = onDocumentCreated({ document: 'leituras/{id}', retry: false, timeoutSeconds: 300 }, async event => {
  ...
  await db.runTransaction(async tx=>{const ref=db.doc('pushInternal/station');const current=(await tx.get(ref)).data();if(!current||value.at>current.at)tx.set(ref,value);});
  await broadcast(value);
});

exports.stationOfflinePush = onSchedule({schedule:'every 1 minutes',timeoutSeconds:300},async()=>{
