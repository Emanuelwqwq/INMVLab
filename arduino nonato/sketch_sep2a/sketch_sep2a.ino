...
#define DHTPIN 4
// DEFINIDO COMO DHT11 CONFORME SEU SENSOR
...
void setup() {
  ...
  configTime(0, 0, "pool.ntp.org"); // Sincroniza o relógio mundial
  
  config.api_key = API_KEY;
  auth.user.email = USER_EMAIL;
  auth.user.password = USER_PASSWORD;
  
  // ---> A LINHA ABAIXO É OBRIGATÓRIA PARA A AUTENTICAÇÃO FUNCIONAR <---
...
void loop() {
  // Envia dados a cada 1 minuto (60.000 ms)
  if (Firebase.ready() && (millis() - tempoAnterior > 60000 || tempoAnterior == 0)) {
    tempoAnterior = millis();
    
    float t = dht.readTemperature();
    float u = dht.readHumidity();
    time_t timestamp = time(nullptr); // Pega a hora exata
