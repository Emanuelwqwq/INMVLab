...
const CACHE_NAME='imnvlab-v42';
const APP_FILES=['./','./index.html','./styles.css','./dashboard.js','./explorer.js','./lumi.webp','./assets/escola-fachada.jpeg','./manifest.json?v=41','./marca-ceti.jpeg?v=41'];
const LAZY_FILES=[...['seco','umido','frio','ameno'].flatMap(f=>['manha','tarde','noite'].map(p=>'./assets/cenarios/'+f+'-'+p+'.webp')),'./assets/lumi-acenando.webp','./assets/ambiente-paisagem.webp','./assets/projeto-logo-claro.jpeg','./assets/projeto-logo-escuro.jpeg',...['entrada','biblioteca','sala','informatica','xadrez'].map(name=>'./assets/escola-'+name+'.jpeg')];
...
self.addEventListener('fetch', event => {
  ...
  if (url.origin !== scopeUrl.origin || ![...APP_FILES,...LAZY_FILES].some(file => new URL(file, scopeUrl).pathname === url.pathname)) return;
  event.respondWith((async () => {
    const cache = await caches.open(CACHE_NAME);
    if(/\.(?:webp|jpeg|png)$/.test(url.pathname)){const cached=await cache.match(event.request);if(cached)return cached;}
